...
class FeedbackAgent:
    ...
    def parse_answer(self, available_actions, text):
        ...
        for action in available_actions:
            if action in text:
                return action
            # Try matching without square brackets
            action_without_brackets = action.replace("[", "").replace("]", "")
            if action_without_brackets in text:
                return action
        self.write_log_to_file('\nThe first action parsing failed!!!')
        ...
        # Try fuzzy matching by comparing components
        for action in available_actions:
            # Remove brackets and split into components
            action_parts = action.replace("[", "").replace("]", "").split()
            text_parts = text.split()
            
            # Check if all action components are in the text
            all_parts_match = all(part in text_parts for part in action_parts)
            if all_parts_match:
                return action
