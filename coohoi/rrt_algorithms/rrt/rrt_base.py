...
class RRTBase(object):
    ...
    def add_edge(self, tree, child, parent):
        ...
        child_key = tuple(child) if isinstance(child, np.ndarray) else child
        parent_key = tuple(parent) if isinstance(parent, np.ndarray) else parent
        self.trees[tree].E[child_key] = parent_key
    ...
    def reconstruct_path(self, tree, x_init, x_goal):
        x_init = tuple(x_init) if isinstance(x_init, np.ndarray) else x_init
        x_goal = tuple(x_goal) if isinstance(x_goal, np.ndarray) else x_goal
        path = [x_goal]
        current = x_goal
        if x_init == x_goal:
            return path
        while True:
            parent = self.trees[tree].E[current]
            parent = tuple(parent) if isinstance(parent, np.ndarray) else parent
            path.append(parent)
            if parent == x_init:
                break
            current = parent
