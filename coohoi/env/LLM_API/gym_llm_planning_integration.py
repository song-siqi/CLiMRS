...
class GymLLMPlanningIntegration:
    ...
    def _run_planning(self, task_description: str):
        try:
            ...
            if self.current_plan:
                print(f"🚀 开始执行LLM规划: {len(self.current_plan)} 个步骤")
                for i, step in enumerate(self.current_plan):
                    print(f"  步骤{i+1}: {step.get('skill', 'N/A')} - {step.get('robot_name', 'N/A')} - {step.get('area_name', 'N/A')}")
            else:
                print("❌ 没有可执行的规划步骤")
                ...
                if self.plan_execution_index < len(self.current_plan):
                    ...
                    if self._can_execute_step(current_step):
                        ...
                        if success:
                            ...
                            print(f"🔄 下一步: {self.plan_execution_index + 1}/{len(self.current_plan)}")
                        else:
    ...
    def _execute_skill_based_step(self, step: Dict) -> bool:
        ...
        if not skill_name or not robot_name:
            print(f"❌ Invalid skill step: missing skill_name or robot_name")
            return False
        
        if skill_name not in SKILL_MAP:
            print(f"❌ Unknown skill: {skill_name}")
            print(f"Available skills: {list(SKILL_MAP.keys())}")
            return False
    ...
    def _execute_agent_based_step(self, step: Dict) -> bool:
        ...
        if 'agent' not in step or 'action' not in step:
            print(f"❌ Invalid agent step: missing agent or action")
            return False
            ...
            if skill_name in ['A', 'B', 'C', 'D', 'E', 'F', 'G']:
                if hasattr(self, 'mobile_robot_stop_flags'):
                    ...
                    if robot_id and self.mobile_robot_stop_flags.get(robot_id, False):
                        print(f"🛑 Robot {robot_name} is stopped due to safety concerns")
                        return False
            
            return True
        except Exception as e:
            print(f"⚠️  Precondition check failed: {e}")
            return True
    ...
    def _validate_skill_execution(self, skill_name: str, pre_state: Dict, post_state: Dict) -> bool:
        ...
        try:
            if skill_name in ['A', 'B', 'C', 'D']:
                print(f"✅ 技能{skill_name}已设置waypoints，立即完成")
                return True
    ...
    def _check_robot_movement_complete(self, skill_name: str) -> bool:
        ...
        try:
            ...
            if robot_id is None:
                print(f"⚠️  无法确定技能 {skill_name} 对应的机器人")
                return True
                ...
                if hasattr(self.task, 'waypoints') and self.task.waypoints is not None:
                    if hasattr(self.task, 'current_wp_idx'):
                        if self.task.current_wp_idx < len(self.task.waypoints):
                            print(f"⏳ Robot1 移动中: {self.task.current_wp_idx}/{len(self.task.waypoints)}")
                            return False
                        else:
                            print(f"✅ Robot1 完成移动，技能 {skill_name} 执行完成")
                            return True
                ...
                if hasattr(self.task, 'waypoints_2') and self.task.waypoints_2 is not None:
                    if hasattr(self.task, 'current_wp_idx_2'):
                        if self.task.current_wp_idx_2 < len(self.task.waypoints_2):
                            print(f"⏳ Robot2 移动中: {self.task.current_wp_idx_2}/{len(self.task.waypoints_2)}")
                            return False
                        else:
                            print(f"✅ Robot2 完成移动，技能 {skill_name} 执行完成")
                            return True
                ...
                if hasattr(self.task, 'waypoints_3') and self.task.waypoints_3 is not None:
                    if hasattr(self.task, 'current_wp_idx_3'):
                        if self.task.current_wp_idx_3 < len(self.task.waypoints_3):
                            print(f"⏳ Robot3 移动中: {self.task.current_wp_idx_3}/{len(self.task.waypoints_3)}")
                            return False
                        else:
                            print(f"✅ Robot3 完成移动，技能 {skill_name} 执行完成")
                            return True
            
            return False
            
        except Exception as e:
            print(f"⚠️  检查机器人移动状态失败: {e}")
            return True
