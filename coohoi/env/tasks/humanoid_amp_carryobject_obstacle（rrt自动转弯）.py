...
# box位置实体（应该和实物对齐），添加实体
    ...
    def rrt_plan(self):
        ...
        if not hasattr(self, '_waypoints_initialized') or not self._waypoints_initialized:
            ...
            for p1, p2 in zip(paths1, paths2):
                ...
                if p2 is None: p2 = []
                paths.append(p1 + p2)
            self.waypoints = torch.tensor(paths[0], device=self.device, dtype=torch.float32)
            self.waypoints_2 = torch.tensor(paths[1], device=self.device, dtype=torch.float32)
            self.waypoints_3 = torch.tensor(paths[2], device=self.device, dtype=torch.float32)
    ...
    def _reset_car_target4(self, env_ids):
        self.waypoints_4 = torch.tensor(
            [
                [-0.5,-5.0],
                [-1.2, -9.0],
                [-5.0, -9.0],
    ...
    def _update_robot_2(self, mobile_robot_2_state_tensor, component_wheel_2_state_tensor,
                    all_root_state, step_size):
        # Retrieve required tensors for pushing logic
        component_wheel_1_state_tensor = all_root_state[-14]
        component_body_state_tensor = all_root_state[-6]
    ...
    def _smart_push_box(self, robot_state, box_state, box_global_idx, target_waypoint, all_root_state, step_size, robot_id=1):
        ...
        box_to_target = target_waypoint - box_pos
        box_to_target_dist = torch.norm(box_to_target)
        
        if box_to_target_dist < 0.1:
            self._move_to_waypoint(robot_state, target_waypoint, all_root_state, step_size, robot_id=robot_id)
            return
        
        box_to_target_dir = box_to_target / box_to_target_dist

        push_offset = 0.6  
        ideal_push_pos = box_pos - box_to_target_dir * push_offset
        
        robot_to_push_pos = ideal_push_pos - robot_pos
        robot_to_push_dist = torch.norm(robot_to_push_pos)

        push_distance_threshold = 0.15
        robot_state_idx = -robot_id
        
        if robot_to_push_dist > push_distance_threshold:
            move_dir = robot_to_push_pos / robot_to_push_dist
            robot_state[0:2] += move_dir * step_size * 1.5
            ...
            print(f"Robot moving to push position. Distance to push pos: {robot_to_push_dist:.3f}")
            
        else:
            robot_to_box = box_pos - robot_pos
            robot_to_box_normalized = robot_to_box / torch.norm(robot_to_box)
            
            alignment = torch.dot(robot_to_box_normalized, box_to_target_dir)
            
            if alignment > 0.7:  
                robot_state[0:2] += box_to_target_dir * step_size
                box_state[0:2] += box_to_target_dir * step_size
                all_root_state[box_global_idx] = box_state

                dx, dy = box_to_target_dir
                ...
                print(f"Robot pushing box. Box to target distance: {box_to_target_dist:.3f}")
            else:
                correction_dir = ideal_push_pos - robot_pos
                correction_dir = correction_dir / torch.norm(correction_dir)
                robot_state[0:2] += correction_dir * step_size * 0.5  
                
                dx, dy = correction_dir
                angle = torch.atan2(dy, dx) + np.pi / 2
                half_angle = angle / 2.0
                w = torch.cos(half_angle)
                z = torch.sin(half_angle)
                ...
                print(f"Robot adjusting position. Alignment: {alignment:.3f}")
    ...
    def pre_physics_step(self, actions):
        ...
        # 最后一个车辆和franka的距离
        dist0 = torch.norm(self.waypoints[-1] - component_wheel_1_state_tensor[0:2])
        dist1 = torch.norm(self.waypoints_2[-1] - component_wheel_1_state_tensor[0:2])
        dist2 = torch.norm(self.waypoints_3[-1] - component_wheel_1_state_tensor[0:2])
        ...
        if dist0 and dist1 and dist2 < 0.5:
