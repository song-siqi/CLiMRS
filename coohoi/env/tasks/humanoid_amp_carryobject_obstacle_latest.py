...
class HumanoidAMPCarryObjectObstacle(humanoid_amp_task.HumanoidAMPTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        # self._robot_target_pos = torch.zeros(self.num_envs, 3).to(self.device)
        # self._robot_target_rot = torch.zeros(self.num_envs, 4).to(self.device)
    ...
    def _load_box_asset(self):
        ...
        for env_id in range(self.num_envs):
            ...
            self.obs_component.append(self.gym.create_box(
                self.sim, box_length*2, box_width*2, box_height*2, asset_options))
...
# box位置实体（应该和实物对齐），添加实体
    ...
    def _reset_box(self, env_ids):
        ...
        self._box_states[env_ids, 0] = torch.tensor(0.5)
        self._box_states[env_ids, 1] = torch.tensor(6.0)
    ...
    def _reset_components(self, env_id, env_ptr):
        ...
        component_handle = self.gym.create_actor(
            env_ptr, self.obs_component[env_id], default_pose, "box", col_group, col_filter, segmentation_id)
    ...
    def control_ik(self, dpose):
        ...
        A = self.j_eef @ j_eef_T + lmbda
        B = dpose
        x = torch.linalg.solve(A, B)
        u = torch.matmul(j_eef_T, x).squeeze(-1)
        return u    
    ...
    def apply_magnetic_force(self):
        ...
        env_ptr = self.envs[0]
        wheel_handle = self.franka_cube_handles[0]
        body_handle = self.franka_body_handles[0]

        wheel_rb_idx = self.gym.find_actor_rigid_body_handle(env_ptr, wheel_handle, "wheel")
        body_rb_idx = self.gym.find_actor_rigid_body_handle(env_ptr, body_handle, "body")

        rb_states = self.gym.acquire_rigid_body_state_tensor(self.sim)
        rb_states = gymtorch.wrap_tensor(rb_states)
        wheel_pos = rb_states[wheel_rb_idx, :3].cpu().numpy()
        body_pos = rb_states[body_rb_idx, :3].cpu().numpy()

        diff = body_pos - wheel_pos
        dist = np.linalg.norm(diff)

        magnet_range = 0.2865   
        print(f"[磁吸调试] wheel_pos: {wheel_pos}, body_pos: {body_pos}, dist: {dist:.4f}")
        ...
        if dist < magnet_range:
            ...
            body_z = root_state[body_root_idx, 2].item()
            wheel_half_height = 0.02  
            target_z = body_z + 0.22 + wheel_half_height  

            root_state[wheel_root_idx, 0:3] = torch.tensor([
                root_state[body_root_idx, 0].item(),
                root_state[body_root_idx, 1].item(),
                target_z
            ], device=root_state.device)

            quat = gymapi.Quat.from_euler_zyx(0, 0, -np.pi)
            quat_np = np.array([quat.x, quat.y, quat.z, quat.w], dtype=np.float32)
            root_state[wheel_root_idx, 3:7] = torch.tensor(quat_np, device=root_state.device, dtype=root_state.dtype)

            self.gym.set_actor_root_state_tensor(self.sim, gymtorch.unwrap_tensor(root_state))
            print("[磁吸调试] 已吸附 wheel 到 body 圆柱体上表面")
            return True
        else:
            print("[磁吸调试] 距离超出磁吸范围，无吸附。")
            return False
    ...
    def _plan_franka_path_to_pre_grasp(self):
        ...
        cube_indices = torch.tensor(self.franka_cube_handles, dtype=torch.long, device=self.device)
        cube_state_tensor = all_root_state[cube_indices]
        ...
        cube_pos = cube_state_tensor[:, 0:3]
        cube_orn = cube_state_tensor[:, 3:7]
    ...
    def _plan_franka_path_to_pre_place(self):
        ...
        cur_orn = self.franka_rb_states[hand_idxs, 3:7]
        cube_quat = np.array([1.0, 0.0, 0.0, 0.0])
        place_pos = torch.tensor([0.45, -2.0, 0.5], device=self.device)
        place_orn = torch.tensor([1.0, 0.0, 0.0, 0.0], device=self.device) 
        pre_place_pos = place_pos + torch.tensor([0.0, -0.04, 0.14], device=self.device)  

        start_pose = Pose(cur_pos[0].cpu().numpy(), cur_orn[0].cpu().numpy())
        end_pose = Pose(pre_place_pos.cpu().numpy(), cube_quat)
        path = interpolate(start_pose, end_pose, num_steps=120)
        self.set_franka_path(path, duration=100.0)
    ...
    def _plan_franka_path_to_place(self):
        ...
        place_pos = torch.tensor([0.45, -2.04, 0.5], device=self.device) 
        place_orn = torch.tensor([1.0, 0.0, 0.0, 0.0], device=self.device)  
        cube_quat = np.array([1.0, 0.0, 0.0, 0.0])

        start_pose = Pose(cur_pos[0].cpu().numpy(), cur_orn[0].cpu().numpy())
        end_pose = Pose(place_pos.cpu().numpy(), cur_orn[0].cpu().numpy())
        path = interpolate(start_pose, end_pose, num_steps=10)
        self.set_franka_path(path, duration=10.0)
    ...
    def _franka_take_and_place_fsm(self):
        ...
        if self.franka_task_stage == 0:
            ...
            finished = self._step_franka_path(step_scale = 1, gap = 0.5, dist_gap = 0.0052)
    ...
    def _build_mobile_robots(self, env_id, env_ptr):
        ...
        mobile_robot = self.gym.create_box(
                self.sim, 0.5, 0.5, 0.5, asset_options)

        # component_handle = self.gym.create_actor(
            # env_ptr, mobile_robot, default_pose, "mobile_robots", col_group, col_filter, segmentation_id)
    ...
    def _reset_car_target(self, env_ids):
        n = len(env_ids)
        random_numbers = torch.rand(
                [n], dtype=self.target_position.dtype, device=self.target_position.device)
        ...
        # 换成英文，强调严格按照规定格式，看作成功率，输出结构化
            ...
            rand_dist = (self._target_dist_max - self._target_dist_min) * torch.rand(
                [n], dtype=self.target_position.dtype, device=self.target_position.device) + self._target_dist_min
            self.target_position[0] = rand_dist * \
                torch.cos(rand_theta) + self._box_states[env_ids, 0]
            self.target_position[1] = rand_dist * \
    ...
    def _compute_reward(self, actions):
        ...
        distance = torch.norm(mobile_robot_state_tensor[0:2] - self.target_position)
        direction = self.target_position - mobile_robot_state_tensor[0:2]
        ...
        if distance > 0.1:
            ...
            self.gym.set_actor_root_state_tensor_indexed(self.sim, gymtorch.unwrap_tensor(all_root_state),
                                                         gymtorch.unwrap_tensor(torch.tensor([14],dtype=torch.int32).to(self.device)), 1)
