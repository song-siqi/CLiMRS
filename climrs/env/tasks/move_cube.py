...
sim_params.dt = 1 / 60  # Simulation time step
sim_params.substeps = 2  # Substeps
sim_params.up_axis = gymapi.UP_AXIS_Z  # Z axis as up direction
sim_params.gravity = gymapi.Vec3(0.0, 0.0, -9.8)  # Gravity
sim_params.use_gpu_pipeline = True  # Use GPU acceleration
sim = gym.create_sim(0, 0, gymapi.SIM_PHYSX, sim_params)

# Set environment grid
spacing = 5.0  # Environment spacing
lower = gymapi.Vec3(-spacing, 0.0, -spacing)
upper = gymapi.Vec3(spacing, spacing, spacing)
num_per_row = 1  # Number of environments per row
num_env = 1
# Create environment
env = gym.create_env(sim, lower, upper, num_per_row)

# Load assets
asset_root = "/home/pjlab/Desktop/urdf_model/carter"  # Asset folder path
asset_file = "Carter_v1.urdf"  # Asset file name (URDF or SDF file)
asset_options = gymapi.AssetOptions()
# asset_options.fix_base = False
# print(help(gym.set_dof_position_target_tensor))
robot_asset = gym.load_asset(sim, asset_root, asset_file, asset_options)
# num_dofs = gym.get_asset_dof_count(robot_asset)
# import pdb; pdb.set_trace()

obstacle_asset3 = []
obstacle_asset3.append(gym.create_box(
                sim, 0.5, 0.5, 0.5, asset_options))
# Set initial position and pose of objects
pose = gymapi.Transform()
# pose.p = gymapi.Vec3(0.0, 0, 0.0)  # Initial position
# pose.r = gymapi.Quat(-0.707107, 0.0, 0.0, 0.707107)  # Initial pose (no rotation)
default_pose4 = gymapi.Transform()
default_pose4.p.x = 0.0
default_pose4.p.y = 1.5
default_pose4.p.z = 0.25
# Create dynamic objects
# car_actor_handle = gym.create_actor(env, obstacle_asset3[0], default_pose4, "cube", 0, 0)
car_actor_handle = gym.create_actor(env, robot_asset, pose, "cube", 0, 0)
# car_actor_handle2 = gym.create_actor(env, robot_asset, default_pose4, "cube", 0, 0)
...
forces = torch.tensor([[0.0, 0.0, 5]]).to(device="cuda:0")

target_pos = torch.tensor([[10.0, 0.0, 0.0]]).to(device="cuda:0")


dof_prop = gym.get_asset_dof_properties(robot_asset)

pd_tar_tensor = torch.tensor([10.0]).to(device="cuda:0")
...
while not gym.query_viewer_has_closed(viewer):
    ...
    # gym.set_dof_position_target_tensor(sim, gymtorch.unwrap_tensor(pd_tar_tensor))
    
    root_state = gym.acquire_actor_root_state_tensor(sim)
    root_states = gymtorch.wrap_tensor(root_state)
    _humanoid_root_states = root_states.view(1, 1, root_state.shape[-1])[..., 0, :]
    # gym.apply_rigid_body_force_at_pos_tensors(sim, gymtorch.unwrap_tensor(forces), None, gymapi.LOCAL_SPACE)
    # gym.set_dof_target_velocity(sim, car_actor_handle, target_velocity)
