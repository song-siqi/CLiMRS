...
class LLMManager:
    ...
    def _init_llm_systems(self):
        if LLM_DEV_REVISION_AVAILABLE:
            ...
            print("WARNING: LLM dev_revision not available, LLM functionality disabled")
    ...
    def _init_arena_multi_agent(self):
        ...
        args.group_agents = True  # Disable grouping for PEFA baseline testing
        ...
        def env_fn():
            ...
            class MockEnv:
                def __init__(self):
                    ...
                    self.use_agent_grouping = True  # Disable for PEFA baseline testing
                ...
                def get_observations(self):
                    try:
                        if hasattr(self, 'get_positions_for_prompt') and callable(self.get_positions_for_prompt):
                            area_positions, agent_positions = self.task.get_positions_for_prompt(0, self.envs[0])
                    except:
                    ...
                    for key in ['mobile_car_1(201)', 'mobile_car_2(202)', 'mobile_car_3(203)']:
                        if hasattr(self, 'task') and hasattr(self.task, 'agent_states'):
                            state = self.task.agent_states.get(key, {'status': 'idle', 'last_action': None})
                        else:
                            state = self.agent_states.get(key, {'status': 'idle', 'last_action': None})
                        print(f"  {key}: {state}")
                    ...
                    for i in range(5): 
                        if i == 0:  # humanoid agent
                            ...
                            if hasattr(self, 'task') and hasattr(self.task, 'component_check_status'):
                                ...
                                completion_status = getattr(self.task, 'component_completion_status', {})
                                
                                # Check if trunk and left wheel are ready
                                ...
                                left_wheel_picked = completion_status.get('left_wheel_picked', False)
                                right_wheel_picked = completion_status.get('right_wheel_picked', False)
                                if trunk_ready and left_wheel_ready and not left_wheel_picked:
                                    available_actions.append("[pick] <franka> (606) pick and place <left wheel> (405) on <trunk> (303)")
                                if trunk_ready and right_wheel_ready and not right_wheel_picked:
                                ...
                                if not left_wheel_picked:
                                    available_actions.append("[pick] <franka> (606) pick and place <left wheel> (405) on <trunk> (303)")
                                if not right_wheel_picked:
                                    available_actions.append("[pick] <franka> (606) pick and place <right wheel> (406) on <trunk> (303)")
                            ...
                            if (agent_state['status'] == 'pushed' and 
                                ...
                                if hasattr(self, 'task') and hasattr(self.task, 'agent_states'):
                                    self.task.agent_states[agent_key] = agent_state
                                else:
            ...
            if self.arena_multi_agent:
                ...
                if allow_update:
                    ...
                    if action and action != "None":
                        ...
                        self._execute_parallel_actions()
    ...
    def run_llm_planning(self, step_count):
        if self.arena_multi_agent:
            try:
                ...
                # Execute LLM planning with complete group execution
    ...
    def _execute_parallel_actions(self):
        if not hasattr(self.arena_multi_agent.env, 'parallel_actions'):
            return
        
        parallel_actions = self.arena_multi_agent.env.parallel_actions
    ...
    def _update_ongoing_movements(self, step_count):
        pass
    ...
    def _any_robot_executing_waypoints(self):
        ...
        if hasattr(task, 'agent_states'):
            ...
            # If any mobile car is still pushing/moving, immediately block LLM
            if any(status in ['pushing', 'moving'] for status in mobile_cars_status):
                return True
            
            # Check if all active cars have completed push, if so reset push mode
        ...
        if hasattr(task, 'llm_action_type'):
            if task.llm_action_type in ["check", "check_completed"]:
                return False  # Allow next LLM planning
            
            # Pick operation needs to wait for completion
                ...
                if (hasattr(task, 'franka_task_stage') and task.franka_task_stage == 0 and 
                    ...
                    return True  # Pick still executing
            
            elif task.llm_action_type in ["pick_completed"]:
                return False  # Pick completed, allow next planning  
 
        
        # Check if any agents are still executing
        if hasattr(task, 'agent_states'):
            for agent_key, agent_state in task.agent_states.items():
                if isinstance(agent_state, dict):
                    status = agent_state.get('status')
                    if status in ['moving', 'pushing', 'executing', 'picking']:
                        return True
        return False
    
    def update_agent_states(self):
        # Agent states now managed by ArenaMultiAgent system
        pass
    ...
    def execute_safety_control(self, collision_risks):
        # Safety control now handled by climrs environment directly
        pass
    
    def get_experiment_report(self):
        """Generate experiment statistics report"""
        ...
        report = {
            "Experiment Statistics": {
                ...
                "Task Completed": stats['task_completed']
            },
            "Stage Success Rate": {
            ...
            "Efficiency Metrics": {
                "Task Completion Rate": f"{task_completion_rate:.1%}",
                "Step Efficiency": f"{efficiency_score:.3f}",
                "LLM Efficiency": f"{llm_efficiency:.3f}"
            },
            "Robustness": {
                "Failure Count": stats['agent_failures'],
                "Recovery Count": stats['successful_retries'],
    ...
    def auto_save_progress(self):
        """Auto-save progress (incremental save to avoid data loss)"""
        try:
    ...
    def update_pick_completion(self, component_name):
        """Update pick operation completion statistics"""
    ...
    def manual_save_experiment(self):
        """Manually save experiment results (user can call at any time)"""
        if hasattr(self.task, 'experiment_stats'):
            json_path, txt_path = self.save_experiment_results(auto_save=False)
            return json_path, txt_path
        else:
    ...
    def _update_llm_context_with_check_results(self):
        ...
        if check_summary:
            ...
            # Avoid duplicate addition of same check information
            if summary_text not in self.task.dialogue_history:
                self.task.dialogue_history += f"\n{summary_text}"
            
            # Dynamically generate available actions for franka agent
    ...
    def get_current_plan(self):
        # Current plan now managed by ArenaMultiAgent system
        return []
    
    def get_plan_execution_index(self):
        # Plan execution index now managed by ArenaMultiAgent system
        return 0
...
class HumanoidAMPCarryObjectObstacle(humanoid_amp_task.HumanoidAMPTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        self.component_completion_status = {
            'left_wheel_picked': False,
            'right_wheel_picked': False
        }
        
        # Experiment statistics system
        import time
        import os
        ...
        # Create experiment results save directory
        self.experiment_save_dir = "./experiment_results"
        ...
        if not os.path.exists(readme_path):
            with open(readme_path, 'w', encoding='utf-8') as f:
                f.write("Experiment Results Folder Description\n")
                f.write("="*30 + "\n\n")
                f.write("This folder contains experimental statistics results from multi-Agent collaboration system\n\n")
                f.write("File Naming Convention:\n")
                f.write("- exp_[timestamp]_[datetime].json: Structured experiment data\n")
                f.write("- exp_[timestamp]_[datetime]_report.txt: Readable experiment report\n\n")
                f.write("Experiment Metrics Explanation:\n")
                f.write("- Move: Success rate of mobile cars reaching component positions\n")
                f.write("- Push: Success rate of components pushed to franka area\n")
                f.write("- Check: Success rate of franka checking component status\n")
                f.write("- Pick: Success rate of franka assembling components\n\n")
                f.write("Task Completion Criteria: Pick>=2 and Check>=3\n")
        ...
        # Add agent state management - track each mobile car's status
        self.agent_states = {
        ...
        # Simple parallel support: record all active robot ids
        ...
        self.target_id_to_robot_mapping = {
            0: 'robot1',    # left wheel (405) -> robot1 handles
            1: 'robot2',    # right wheel (406) -> robot2 handles  
            2: 'robot3',    # trunk (303) -> robot3 handles
        }
        ...
        self.llm_robot_name_mapping = {
            ...
            'mobile_car': 'robot1',      # backward compatible with old format
            'humanoid': 'humanoid',      
            'franka': 'franka',          # franka keeps original name
            'robot_arm': 'franka',       # robot_arm is also franka
        }
...
# path planning
        ...
        if self.franka_task_stage == 0:
            ...
            if finished:
                ...
                if hasattr(self, 'current_target_name'):
                    if 'left wheel' in str(self.current_target_name).lower() or 'left_wheel' in str(self.current_target_name).lower():
                        self.component_completion_status['left_wheel_picked'] = True
                        print(f"SUCCESS: Left wheel pick completed and marked as picked")
                    elif 'right wheel' in str(self.current_target_name).lower() or 'right_wheel' in str(self.current_target_name).lower():
                        self.component_completion_status['right_wheel_picked'] = True
                        print(f"SUCCESS: Right wheel pick completed and marked as picked")
    ...
    def plan_franka_path_to_pre_place(self):
        ...
        cube_quat = np.array([0.5, 0.5, 0.5, 0.5]) # right
        place_pos = all_root_state[body_root_idx, 0:3] 
        ...
        if self.franka_task_stage_1 == 0:
            ...
            if finished:
                ...
                if hasattr(self, 'current_target_name'):
                    if 'left wheel' in str(self.current_target_name).lower() or 'left_wheel' in str(self.current_target_name).lower():
                        self.component_completion_status['left_wheel_picked'] = True
                        print(f"SUCCESS: Left wheel pick completed and marked as picked (stage_1)")
                    elif 'right wheel' in str(self.current_target_name).lower() or 'right_wheel' in str(self.current_target_name).lower():
                        self.component_completion_status['right_wheel_picked'] = True
                        print(f"SUCCESS: Right wheel pick completed and marked as picked (stage_1)")
...
# Mobile robots
    ...
    def fix_component_2_quat(self):
        ...
        root_state = gymtorch.wrap_tensor(root_state)
        idx = self.gym.get_actor_index(env_ptr, handle, gymapi.DOMAIN_SIM)
        root_state[idx, 3:7] = torch.tensor([0.7071, -0.7071, 0.0, 0.0], device=root_state.device, dtype=root_state.dtype)
    ...
    def _build_mobile_robots_3(self, env_id, env_ptr):
        ...
        asset_root = "Agent"  
        asset_file = "tracer_mini/urdf/tracer_mini.urdf"  # Asset file name (URDF or SDF file)
    ...
    def _update_mobile_robots(self):
        ...
        robot_1_idx = self.gym.get_actor_index(env_ptr, self.mobile_handles[0], gymapi.DOMAIN_SIM)
        robot_2_idx = self.gym.get_actor_index(env_ptr, self.mobile_handles[1], gymapi.DOMAIN_SIM)
        robot_3_idx = self.gym.get_actor_index(env_ptr, self.mobile_handles[2], gymapi.DOMAIN_SIM)
        
        left_wheel_idx = self.gym.get_actor_index(env_ptr, self.component_cube_handles[0], gymapi.DOMAIN_SIM)
        right_wheel_idx = self.gym.get_actor_index(env_ptr, self.component_cube_handles[1], gymapi.DOMAIN_SIM)
        trunk_idx = self.gym.get_actor_index(env_ptr, self.component_cube_handles[2], gymapi.DOMAIN_SIM)
        
        mobile_robot_state_tensor = all_root_state[robot_1_idx]
        mobile_robot_2_state_tensor = all_root_state[robot_2_idx]
        mobile_robot_3_state_tensor = all_root_state[robot_3_idx]
        component_wheel_1_state_tensor = all_root_state[left_wheel_idx]
        component_wheel_2_state_tensor = all_root_state[right_wheel_idx]
        component_body_state_tensor = all_root_state[trunk_idx]
    ...
    def _update_robot_1(self, mobile_robot_state_tensor, component_wheel_1_state_tensor,
        ...
        component_idxs = [left_wheel_idx, right_wheel_idx, trunk_idx]
...
## humanoid carry object obstacle ##
    def _reset_target(self, env_ids):
        ...
        if self.is_ask_llm:
            answer = self.ask_llm(f"now waypoint:[3.5,7.5]at{self._box_states[env_ids, 0]}")
...
## LLM Integrated ##
    ...
    def _set_current_context(self, robot_name, robot_id, target_name=""):
        ...
        # Auto-assign target based on robot_id for mobile_car
        if robot_id == 201:
    ...
    def walk_humanoid(self, robot_name, area_name):
        ...
        # Humanoid uses _target_pos, not waypoints like mobile cars
        area_targets = {
            'A': [4.0, 8.0],
            'B': [-4.0, 8.0], 
            'C': [-4.0, -8.0],
        ...
        if area_name in area_targets:
            target_coords = area_targets[area_name]
            # Set humanoid target position directly
    ...
    def _update_mobile_robots_llm_mode(self):
        ...
        mobile_cars_active = [rid for rid in self.active_robot_ids if 201 <= rid <= 203]
        
        if mobile_cars_active:
            robots_to_remove = []
            for robot_full_id in mobile_cars_active:
                simple_robot_id = robot_full_id - 200
                agent_key = f'mobile_car_{simple_robot_id}({robot_full_id})'
                if hasattr(self, 'agent_states') and agent_key in self.agent_states:
                    status = self.agent_states[agent_key].get('status', 'idle')
                    if status == 'pushed':  
                        robots_to_remove.append(robot_full_id)
                        continue
