...
def plot_obstacles(X_dimensions, obstacles, start, goal, path=None):
    ...
    ax.plot(goal[0], goal[1], 'ro')    # End point
    if path is not None:
        path = np.array(path)
...
def move_along_axis(current, target, obstacles, axis):
    """
    Move along specified axis direction to target position
    axis: 0 for x, 1 for y
...
def plan_path_to_franka(gym, sim, device, env_ptr, root_state, robot_handle, box_handle, all_obstacle_handles, max_samples=1024, custom_push_target=None,start_pos=None, force_axis_aligned=True):
    ...
    if force_axis_aligned:
        ...
        if use_axis:
            ...
            if len(path) >= 2:
                prev = path[-1]
                    # Ensure dimension matching: box_center is 2D, prev is 3D, take first two dimensions
                if len(prev) == 3:
                    prev_2d = prev[:2]
                else:
                    prev_2d = prev
                ...
                if norm > 1e-3:
                    ...
                    # Expand 2D coordinates to 3D (z=0)
